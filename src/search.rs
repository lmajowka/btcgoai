...
fn bytes_equal(a: &[u8], b: &[u8]) -> bool {
    ...
    // Usando iteração e verificação byte-a-byte com SIMD implícito
...
pub fn search_for_private_key(min_key: &BigUint, max_key: &BigUint, target_hash160: &[u8]) {
    ...
    // Configurar pool de threads para usar todos os cores físicos e lógicos
    ...
    println!("{}Usando {} threads para busca paralela (100% dos cores lógicos){}", colors::GREEN, num_threads, colors::RESET);
    ...
    // Usar ThreadPool com nossa thread pool configurada
    thread_pool.install(|| {
        // Processamento paralelo dos chunks
        chunks.par_iter().for_each(|(chunk_min, chunk_max)| {
            let mut current_key = chunk_min.clone();
            // Tamanho de batch otimizado para melhor desempenho de cache
            let batch_size = 4096; // Maior batch size para reduzir overhead
            ...
            while &current_key <= chunk_max {
                ...
                for key in &batch {
                    ...
                    // Usar processamento inline
                    if let Ok(hash160) = private_key_to_hash160(&padded_key) {
                        // Verificar se corresponde ao hash160 alvo com comparação otimizada
                    ...
                    // CPU yield para permitir troca de contexto mais eficiente
                    if batch.len() % 128 == 0 {
                        spin_loop();
                    }
                }
...
fn search_key_range_cpu(
    ...
    // Usar um batch_size maior para reduzir overhead de alocação
    let optimized_batch_size = std::cmp::max(batch_size, 8192);
    
    // Process keys in batches
    let mut keys_buffer = Vec::with_capacity(optimized_batch_size);
    let increment = BigUint::from(optimized_batch_size);
    ...
    while current_key <= *max_key {
        ...
        // Process this batch with Rayon parallel iterator para usar todas as CPU cores
        let found_key = keys_buffer.par_iter().find_any(|key| {
            ...
            // Validate the key
            validate_private_key_for_hash160(&key_bytes, target_hash160)
        });
...
pub fn search_for_private_key_optimized(
    ...
    // Configurar Rayon para usar todos os cores disponíveis
    let num_threads = num_cpus::get();
    let thread_pool = ThreadPoolBuilder::new()
        .num_threads(num_threads)
        .stack_size(8 * 1024 * 1024) // 8MB stack
    ...
    println!("{}Configurado para usar {} threads (100% dos cores lógicos){}", 
             crate::colors::GREEN, num_threads, crate::colors::RESET);
    
    // Initialize a start time for this search
    let search_start_time = Instant::now();
    
    // Fix lifetime issue by using a simple value wrapped in Arc::new
    let found_key = Arc::new(Mutex::new(Option::<String>::None));
    ...
    // Primeiro tentar com GPU (se disponível) - Single-threaded para evitar problemas com OpenCL
    #[cfg(feature = "opencl")]
    {
        if let Some(mut searcher) = gpu_searcher {
            println!("{}Tentando com GPU primeiro...{}", 
                     crate::colors::BOLD_GREEN, crate::colors::RESET);
            
            // Converter target_hash160 para array
            let mut target_array = [0u8; 20];
            target_array.copy_from_slice(target_hash160);
            
            // Processar alguns chunks iniciais com GPU
            let max_gpu_chunks = std::cmp::min(total_ranges, 10); // Limitar a 10 chunks para não bloquear CPU por muito tempo
            
            for i in 0..max_gpu_chunks {
                let (min, max) = &search_ranges[i];
                
                // Se a chave já foi encontrada, não continuar
                if found_key.lock().unwrap().is_some() {
                    break;
                }
                
                // Converter para u64 se possível
                if let (Some(min_u64), Some(max_u64)) = (min.to_u64(), max.to_u64()) {
                    println!("{}GPU: Processando chunk {}/{}...{}", 
                             crate::colors::CYAN, i+1, max_gpu_chunks, crate::colors::RESET);
                    ...
                    match searcher.search_direct(&target_array, min_u64, max_u64, batch_size) {
                        ...
                        Err(e) => {
                            println!("{}Erro na GPU: {}, usando CPU{}", 
                                     crate::colors::YELLOW, e, crate::colors::RESET);
                            break; // Sair do loop GPU e continuar com CPU
                        }
                    }
                }
            }
    ...
    // Se não encontrou com GPU, continuar com CPU em paralelo
    if found_key.lock().unwrap().is_none() {
        println!("{}Continuando busca com CPU paralela...{}", 
                 crate::colors::BOLD_GREEN, crate::colors::RESET);
                 
        // Criar uma lista de índices para os chunks que ainda não foram processados
        let mut remaining_chunks = Vec::new();
        
        #[cfg(feature = "opencl")]
        {
            // Se usamos GPU, pular os primeiros chunks que já foram processados
            let max_gpu_chunks = std::cmp::min(total_ranges, 10);
            for i in max_gpu_chunks..search_ranges.len() {
        ...
        #[cfg(not(feature = "opencl"))]
        {
            // Se não usamos GPU, processar todos os chunks
            for i in 0..search_ranges.len() {
                remaining_chunks.push(i);
            }
        }
        
        // Usar thread pool otimizada para CPU
        thread_pool.install(|| {
            // Processar chunks em paralelo quando possível
            remaining_chunks.par_iter().for_each(|&i| {
                // Se já encontramos uma chave, não continuar buscando
                {
                    let key_guard = found_key.lock().unwrap();
                ...
                // Try to process this chunk with CPU
                let chunk_result = search_key_range_cpu(
                    min, max, target_hash160, 
                    std::cmp::max(batch_size, 8192) // Usar batch size maior para melhor desempenho
                );
    ...
    // Retornar a chave encontrada (se houver) - evitar o problema de lifetime
    let result_guard = found_key.lock().unwrap();
    result_guard.clone()
}
