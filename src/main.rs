...
fn main() {
    ...
    // Perguntar se o usuário quer executar em modo de teste
    print!("{}Deseja executar em modo de teste com puzzles menores? (S/N): {}", colors::CYAN, colors::RESET);
    io::stdout().flush().unwrap();
    
    let stdin = io::stdin();
    let test_mode_str = stdin.lock().lines().next().unwrap().unwrap();
    
    if test_mode_str.trim().eq_ignore_ascii_case("s") || 
       test_mode_str.trim().eq_ignore_ascii_case("sim") ||
       test_mode_str.trim().eq_ignore_ascii_case("y") ||
       test_mode_str.trim().eq_ignore_ascii_case("yes") {
        // Modo de teste com puzzles pequenos
        run_test_mode(num_threads);
    } else {
        // Modo normal com puzzles reais
        run_normal_mode(num_threads);
    }
}

fn run_test_mode(num_threads: usize) {
    println!("{}Modo de TESTE ativado - Usando puzzles pequenos para verificação{}", colors::BOLD_GREEN, colors::RESET);
    ...
    println!("{}Carregados {} puzzles de teste{}", colors::GREEN, test_puzzles.len(), colors::RESET);

    // Prompt user para escolher o puzzle
    println!("\n{}Puzzles de teste (baixa dificuldade para verificação):{}", colors::BOLD_YELLOW, colors::RESET);
